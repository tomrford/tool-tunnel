...
The recommended shape is a remote MCP-aware exporter plus local MCP aggregator. The exporter wraps one stdio MCP server, maintains one child MCP client session, and exposes tools over MCP sessions carried by iroh streams. The local aggregator is the only process configured in the MCP client; it reads remote tickets, dials exporters, starts MCP client sessions over iroh, fetches tool manifests, prefixes tools, and forwards calls. Trust is endpoint-ID based, with allowlists on both ends.
...
`tool-tunnel` is not a wrapper around the dumbpipe CLI. It copies the endpoint/ticket/stream lifecycle patterns and puts MCP-specific behavior in the proxy layer: stdio JSON-RPC discipline, MCP client sessions over remote iroh streams, tool-list aggregation, name prefixing, trusted-peer config, serialized child calls, and structured errors.
...
Custom transports are allowed, but the practical compatibility path is to keep the client-facing side stdio. `tool-tunnel client` is therefore a stdio MCP server even though it uses iroh internally.

The remote exporter uses `rmcp` over the iroh stream as the private transport between `tool-tunnel client` and `tool-tunnel export`. The child MCP server still speaks normal stdio MCP and sees only the export process as its client.
